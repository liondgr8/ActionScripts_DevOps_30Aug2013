...
if (isset($options["load_balancer_name"]) && isset($options["policy_name"]) && isset($options["policy_type_name"])) {
    $load_balancer_name = $options["load_balancer_name"];
    $policy_name = $options["policy_name"];
    $policy_type_name = $options["policy_type_name"];
    $opt = array();


    if (isset($options['PolicyAttributes'])) {
        $opt['PolicyAttributes'] = $options['PolicyAttributes'];
    ...
    $response = $elb->create_load_balancer_policy($load_balancer_name, $policy_name, $policy_type_name, $opt);
