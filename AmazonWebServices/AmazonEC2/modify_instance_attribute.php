<?php
/**
 * User: Suraj.Savita
 * Date: 6/26/12
 * Time: 3:35 PM
 */


error_reporting(-1);

// Set plain text headers
header("Content-type: text/plain; charset=utf-8");

// Include the SDK


ini_set('include_path', ini_get('include_path') . ';C:\wamp\www');
require_once '/sdk-1.5.7/sdk.class.php';
require_once '/include/get_account_name_creds.php';

$shortopts = "abc"; // These options do not accept values

$longopts = array(
    "instance_id:",
    "Attribute::",
    "Value::",
    "BlockDeviceMapping::",
    "SourceDestCheck.Value::",
    "DisableApiTermination.Value::",
    "InstanceType.Value::",
    "Kernel.Value::",
    "Ramdisk.Value::",
    "UserData.Value::",
    "InstanceInitiatedShutdownBehavior.Value::",
    "GroupId::",
    "curlopts::",
    "returnCurlHandle::",
);
$options = getopt($shortopts, $longopts);


$options = $params;

$err = "Not sufficient arguments";
$creds = array();
$creds['key'] = $access_key;
$creds['secret'] = $secret_key;
//    print_r($creds);
$ec2 = new AmazonEC2($creds);
if (isset($options["instance_id"])) {
    $instance_id = $options["instance_id"];
    $opt = array();

    if (isset($options['Attribute'])) {
        $opt['Attribute'] = $options['Attribute'];

    }
    if (isset($options['Value'])) {
        $opt['Value'] = $options['Value'];

    }
    if (isset($options['BlockDeviceMapping'])) {
        $opt['BlockDeviceMapping'] = $options['BlockDeviceMapping'];

    }
    if (isset($options['SourceDestCheck.Value'])) {
        $opt['SourceDestCheck.Value'] = $options['SourceDestCheck.Value'];

    }
    if (isset($options['DisableApiTermination.Value'])) {
        $opt['DisableApiTermination.Value'] = $options['DisableApiTermination.Value'];

    }
    if (isset($options['InstanceType.Value'])) {
        $opt['InstanceType.Value'] = $options['InstanceType.Value'];

    }
    if (isset($options['Kernel.Value'])) {
        $opt['Kernel.Value'] = $options['Kernel.Value'];

    }
    if (isset($options['Ramdisk.Value'])) {
        $opt['Ramdisk.Value'] = $options['Ramdisk.Value'];

    }
    if (isset($options['UserData.Value'])) {
        $opt['UserData.Value'] = $options['UserData.Value'];

    }
    if (isset($options['InstanceInitiatedShutdownBehavior.Value'])) {
        $opt['InstanceInitiatedShutdownBehavior.Value'] = $options['InstanceInitiatedShutdownBehavior.Value'];

    }
    if (isset($options['GroupId'])) {
        $opt['GroupId'] = $options['GroupId'];

    }
    if (isset($options['curlopts'])) {
        $opt['curlopts'] = $options['curlopts'];

    }
    if (isset($options['returnCurlHandle'])) {
        $opt['returnCurlHandle'] = $options['returnCurlHandle'];

    }


    $response = $ec2->modify_instance_attribute($instance_id, $opt);
//        print_r($response->isOK());
//        print_r($response);

    $file = fopen(dirname(__FILE__) . '/' . basename(__FILE__, '.php') . '-' . time() . ".json", 'w');
    fwrite($file, json_encode($response));
    fclose($file);

    return $response;

} else {
//    print_r($err);

    return $err;

}
